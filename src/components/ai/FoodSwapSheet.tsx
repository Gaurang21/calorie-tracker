...
import { suggestFoodSwap } from '../../services/aiService';
import type { FoodLog } from '../../types/db';
import type { MacroGaps, FoodSwap } from '../../types/ai';

interface Props {
  open: boolean;
  onClose: () => void;
  food: FoodLog | null;
  macroGaps: MacroGaps | null;
  onSwap: (s: FoodSwap) => void;
}

export default function FoodSwapSheet({ open, onClose, food, macroGaps, onSwap }: Props) {
  const [swaps, setSwaps] = useState<FoodSwap[] | null>(null);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{food ? `Smarter swaps for ${food.name}` : 'Swaps'}</DialogTitle>
        </DialogHeader>
              ...
              return (
                <div key={i} className="rounded-xl p-3" style={{ backgroundColor: 'var(--surface-2)' }}>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Button data-testid={`swap-${i}`} size="sm" onClick={() => onSwap(s)}>Log instead</Button>
                  </div>
                </div>
              );
            })}
