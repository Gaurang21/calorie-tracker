...
export default function MealTemplatesTab({ meal, onSaveMany }) {
  const { user } = useAuth();
  ...
  const logTemplate = async () => {
    if (!selected) return;
    setBusy(true);
    const rows = (selected.foods || []).map((f) => ({
      meal,
      name: f.name,
      calories: f.calories,
  ...
  const totalCals = (t) => (t.foods || []).reduce((acc, f) => acc + (f.calories || 0), 0);
