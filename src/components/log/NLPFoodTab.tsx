...
export default function NLPFoodTab({ meal, onSaveMany }: Props) {
  ...
  const parse = async (): Promise<void> => {
    ...
    try {
      ...
      setError((e as Error).message || 'AI features unavailable — check your server connection');
    } finally {
      setBusy(false);
    }
  };

  const updateItem = (idx: number, patch: Partial<ParsedFoodItem>): void => {
    setItems((prev) => prev?.map((it, i) => (i === idx ? { ...it, ...patch } : it)) ?? null);
  };
  const removeItem = (idx: number): void => setItems((prev) => prev?.filter((_, i) => i !== idx) ?? null);
  ...
  const logAll = async () => {
    ...
    setLogging(true);
    try {
      const rows: Partial<FoodLog>[] = items.map((it) => ({
        meal,
        name: it.name,
          ...
          {items.map((it, i) => (
            <div key={i} className="card p-3 space-y-2">
              ...
              <div className="grid grid-cols-4 gap-2 text-sm">
                <Field label="kcal" value={it.calories} onChange={(v) => updateItem(i, { calories: Number(v) })} />
                <Field label="P" value={it.protein_g} onChange={(v) => updateItem(i, { protein_g: Number(v) })} />
                <Field label="C" value={it.carbs_g} onChange={(v) => updateItem(i, { carbs_g: Number(v) })} />
                <Field label="F" value={it.fat_g} onChange={(v) => updateItem(i, { fat_g: Number(v) })} />
              </div>
            </div>
          ))}
