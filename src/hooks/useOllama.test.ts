...
const testConnectionMock = testConnection as unknown as {
  mockClear: () => void;
  mockResolvedValueOnce: (v: unknown) => void;
};

describe('useOllama', () => {
  beforeEach(() => { testConnectionMock.mockClear(); });
  ...
  it('check() can override url/apiKey', async () => {
    testConnectionMock.mockResolvedValueOnce({ ok: false, error: 'no' });
    const { result } = renderHook(() => useOllama());
    await waitFor(() => expect(result.current.status.checked).toBe(true));
    await act(async () => {
