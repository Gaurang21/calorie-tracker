...
export default function AIChat() {
  ...
  const [messages, setMessages] = useState([]); // { role: 'user' | 'ai', content, error? }
  ...
  const buildContext = async () => {
    ...
    const avgCalories = days.length ? Math.round(days.reduce((a, d) => a + foodByDay[d].calories, 0) / days.length) : 0;
    const avgProtein = days.length ? Math.round(days.reduce((a, d) => a + foodByDay[d].protein_g, 0) / days.length) : 0;
