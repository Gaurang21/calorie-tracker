...
async function loadService() {
  return await import('./groqService');
}

function mockChat(content: string, ok = true, status = 200): void {
...
describe('groqService', () => {
  ...
  it('uses Authorization Bearer header for Groq', async () => {
    ...
    const [, init] = (fetchMock as unknown as { mock: { calls: any[][] } }).mock.calls[0];
  ...
  it('uses chat/completions endpoint', async () => {
    ...
    expect((fetchMock as unknown as { mock: { calls: any[][] } }).mock.calls[0][0]).toMatch(/\/chat\/completions$/);
  });
  ...
  it('passes response_format=json_object when expectJson is true', async () => {
    ...
    const body = JSON.parse((fetchMock as unknown as { mock: { calls: any[][] } }).mock.calls[0][1].body);
  ...
  it('throws on non-OK response', async () => {
    const svc = await loadService();
    mockChat('', false, 401);
    await expect(svc.generateWeeklySummary({
      daysLogged: 0, avgCalories: 0, calorieGoal: 0, daysHitGoal: 0,
      avgProtein: 0, proteinGoal: 0, weightChange: 0, activitySessions: 0, caloriesBurned: 0,
    })).rejects.toThrow(/401/);
  });
  ...
  describe('testConnection', () => {
    ...
    it('returns ok=false on HTTP error', async () => {
      const svc = await loadService();
      (fetchMock as unknown as { mockResolvedValueOnce: (v: unknown) => void }).mockResolvedValueOnce({ ok: false, status: 401, json: async () => ({}) });
